...
constexpr double ref_v = 20* 0.44704; // in m/s
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N - 1; i++) {
        ...
        AD<double> delta0 = (i>=0) ? vars[delta_start + i] : 0.0;
        AD<double> a0 = (i>=0) ? vars[a_start + i] : 0.0;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //
  // Object that computes objective and constraintss
  ...
  // Return the actuator values of the first time step only
